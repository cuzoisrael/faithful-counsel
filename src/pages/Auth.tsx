...
const Auth = () => {
  ...
  const recordAttempt = async (success: boolean) => {
    try {
      const { data } = await supabase.rpc("record_signin_attempt", {
        _email: email,
        _success: success,
      });
  ...
  const checkLocked = async () => {
    const { data } = await supabase.rpc("check_signin_lockout", { _email: email });
    ...
    if (row?.locked) {
      ...
      toast.error(
        until
          ...
          : "Too many failed attempts. Please wait 30 minutes and try again."
      );
      return true;
    }
    return false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        const lockUntil = await recordAttempt(false);
        if (lockUntil) {
  ...
  const handleVerifyMfa = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await supabase.auth.mfa.verify({ factorId, challengeId, code: otp.trim() });
    setLoading(false);
    if (error) {
            ...
            {step === "mfa" ? (
              ...
              <>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-input bg-card text-foreground text-sm font-medium hover:bg-secondary transition-colors mb-4"
                >
